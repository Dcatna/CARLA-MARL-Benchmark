...
config_dir = os.path.join(os.path.dirname(__file__), 'configs', 'configs_ppo.ini')
config = configparser.ConfigParser()

# Read the configuration file
if not config.read(config_dir):
    raise FileNotFoundError(f"Configuration file not found: {config_dir}")

# Ensure the 'MODEL_CONFIG' section exists
if 'MODEL_CONFIG' not in config:
    raise configparser.NoSectionError('MODEL_CONFIG')

# Fetch the torch seed
...
class MAPPO:
    ...
    def __init__(self, env, state_dim, action_dim,
        ...
        self.env_state, self.action_mask = self.reset_environment()
        self.n_episodes = 0
        self.n_steps = 0
    ...
    def reset_environment(self):
        # Implement your CARLA environment reset logic here
        #self.env.reset()
    ...
    def get_initial_state(self):
        # Implement logic to get the initial state from CARLA
        return np.zeros(self.state_dim)  # Example placeholder

    def get_action_mask(self):
        # Implement logic to get the action mask from CARLA
        return np.ones(self.action_dim)  # Example placeholder
